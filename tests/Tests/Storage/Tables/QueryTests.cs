...
public class QueryTests
{
    ...
    public void Query_Entities_With_Linq_Should_Succeed()
    {
        ...
        tableClient.AddEntity(new TableEntity("pkA", "rk1"));
        tableClient.AddEntity(new TableEntity("pkA", "rk2"));
        tableClient.AddEntity(new TableEntity("pkB", "rk1"));

        var entities = tableClient.Query<TableEntity>(e => e.PartitionKey == "pkA");

        entities.Should().HaveCount(2).And.OnlyContain(e => e.PartitionKey == "pkA");
    }


    [TestMethod]
    [DataRow("PartitionKey eq 'pkA'", 2, DisplayName = "Range query for single condition on standard property.")]
    [DataRow("PartitionKey eq 'pkA' and RowKey eq 'rk1'", 1, DisplayName = "Simple point query with some results.")]
    [DataRow("PartitionKey eq 'pkC' and RowKey eq 'rk1'", 0, DisplayName = "Simple point query without results.")]
    [DataRow("PartitionKey eq 'pkA' and (RowKey eq 'rk1' or RowKey eq 'rk2')", 2, DisplayName = "OR point query with all results.")]
    [DataRow("PartitionKey eq 'pkA' and (RowKey eq 'rk1' or RowKey eq 'rk4')", 1, DisplayName = "OR point query with some results.")]
    [DataRow("PartitionKey eq 'pkA' and (RowKey eq 'rk3' or RowKey eq 'rk4')", 0, DisplayName = "OR point query without results.")]
    [DataRow("customProperty1 eq 'test'", 1, DisplayName = "Range query on custom string property.")]
    [DataRow("customProperty2 eq 42", 1, DisplayName = "Range query on custom integer property.")]
    [DataRow("customProperty3 eq 42.42d", 1, DisplayName = "Range query on custom double property.")]
    [DataRow("PartitionKey eq 'pkA' and RowKey ge 'rk1' and RowKey lt 'rk3'", 2, DisplayName = "Range query with RowKey range condition.")]
    public void Query_Entities_With_String_Should_Succeed(string query, int numberOfResults)
    {
        using var loggerFactory = LoggerFactory.Create(builder => builder.AddSimpleConsole(o => o.IncludeScopes = true));

        var account = new InMemoryStorageProvider(loggerFactory: loggerFactory).AddAccount();

        var tableClient = InMemoryTableClient.FromAccount(account, "TestTable");

        tableClient.Create();

        tableClient.AddEntity(new TableEntity("pkA", "rk1") { ["customProperty1"] = "test" });
        tableClient.AddEntity(new TableEntity("pkA", "rk2") { ["customProperty2"] = 42 });
        tableClient.AddEntity(new TableEntity("pkB", "rk1") { ["customProperty3"] = 42.42d });
