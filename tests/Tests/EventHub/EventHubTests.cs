...
public class EventHubTests
{
    ...
    public void ConnectionString_ShouldBeReturned()
    {
        ...
        connection.FullyQualifiedNamespace.Should().Be(eventHub.Namespace.Hostname);
    ...
    public async Task InitialSequenceNumbers_WithRandomization_WithSeed_ShouldBeUsed(int? seed, int? min, int? max, int expectedSequenceNumberForPartition0, int expectedSequenceNumberForPartition1)
    {
        ...
        await using var consumer0 = InMemoryPartitionReceiver.FromEventHub("$default", "0", eventHub, EventPosition.Earliest);
        await using var consumer1 = InMemoryPartitionReceiver.FromEventHub("$default", "1", eventHub, EventPosition.Earliest);

        var sentEventData = new EventData();

        producer.Send(sentEventData, new SendEventOptions { PartitionId = "0" });
        producer.Send(sentEventData, new SendEventOptions { PartitionId = "1" });
    ...
    public async Task InitialSequenceNumbers_WithRandomization_WithoutSeed_ShouldBeUsed()
    {
        ...
        {
            producer.Send(new EventData(), new SendEventOptions { PartitionId = "0" });
        }
